"""
dcf_comps_build_v2.py - DCF / Comps Equity Research Excel Generator (V2)
Extends V1 with support for:
  - Feature A: SGA Absolute Projection (for loss-making companies)
  - Feature B: NOPAT Floor Tax Logic (MAX(0, ...) for negative EBIT)
  - Feature C: EV/Sales Primary Multiple (for high-growth tech)
All features are controlled via optional "advanced_settings" in config.
Without advanced_settings, output is identical to V1.
Edit the config dict below, then run:  python dcf_comps_build_v2.py
"""
...
config = {
    ...
    # ── V2 Advanced Settings (optional — remove or leave empty for V1 behavior) ──
    # "advanced_settings": {
    #     "sga_projection_method": "absolute",   # "percent" (default/V1) or "absolute"
    #     "tax_logic": "nopat_floor",             # "standard" (default/V1) or "nopat_floor"
    #     "primary_multiple": "EV/Sales",         # "EV/EBITDA" (default/V1) or "EV/Sales"
    # },
    # "proj_sga_absolute": [15000, 15500, 16000, 16500, 17000],  # required when sga_projection_method == "absolute"
}
...
# =====================================================================
# V2: ADVANCED SETTINGS PARSE
# =====================================================================
_adv = config.get("advanced_settings", {})
SGA_METHOD   = _adv.get("sga_projection_method", "percent")
TAX_LOGIC    = _adv.get("tax_logic", "standard")
PRIMARY_MULT = _adv.get("primary_multiple", "EV/EBITDA")

USE_SGA_ABSOLUTE = (SGA_METHOD == "absolute")
USE_TAX_FLOOR    = (TAX_LOGIC == "nopat_floor")
USE_EV_SALES     = (PRIMARY_MULT == "EV/Sales")

SGA_OFFSET = 2 if USE_SGA_ABSOLUTE else 0  # SGA row + Implied Margin row

# V2: Config validation for SGA absolute mode
if USE_SGA_ABSOLUTE:
    assert "proj_sga_absolute" in config, \
        "advanced_settings.sga_projection_method='absolute' requires 'proj_sga_absolute' in config"
    assert len(config["proj_sga_absolute"]) == config["projection_years"], \
        f"proj_sga_absolute length ({len(config['proj_sga_absolute'])}) must match projection_years ({config['projection_years']})"

# =====================================================================
# V2: DYNAMIC ROW NUMBERS (offset by SGA_OFFSET when SGA absolute is active)
# =====================================================================
R_REVENUE    = 30
R_SGA        = 31 if USE_SGA_ABSOLUTE else None
R_OP_M_IMPL  = 32 if USE_SGA_ABSOLUTE else None
R_EBIT       = 31 + SGA_OFFSET
R_TAX        = 32 + SGA_OFFSET
R_NOPAT      = 33 + SGA_OFFSET
R_DA         = 34 + SGA_OFFSET
R_CAPEX      = 35 + SGA_OFFSET
R_UFCF       = 36 + SGA_OFFSET
R_DISC       = 37 + SGA_OFFSET
R_PV_FCF     = 38 + SGA_OFFSET
...
def calc_dcf_pgm(rev_growth, op_margin, wacc, tg, cfg):
    ...
    # V2: Tax floor logic
    tax_floor = cfg.get("advanced_settings", {}).get("tax_logic", "standard") == "nopat_floor"
    ...
    for yr_idx, rev in enumerate(revenues):
        ebit = rev * op_margin
        # V2: Feature B — NOPAT floor
        if tax_floor and ebit < 0:
            nopat = ebit  # Tax = 0 when EBIT is negative
        else:
            nopat = ebit * (1 - tax)
        da = rev * da_pct
        capex = rev * capex_pct
...
output_file = f"{ticker_safe}_Equity_Research_V2.xlsx"
...
# DCF - Perpetuity Growth (V2: dynamic row reference)
...
# DCF - Exit Multiple (V2: dynamic row reference)
...
# V2: Feature C — Comps label depends on primary multiple
if USE_EV_SALES:
...
# V2: FCF row labels using dynamic R_ variables
row_labels_fcf = [
    ("Revenue",                       R_REVENUE),
]
if USE_SGA_ABSOLUTE:
    row_labels_fcf.append(("SGA Expense",               R_SGA))
    row_labels_fcf.append(("Implied Operating Margin",  R_OP_M_IMPL))
row_labels_fcf.extend([
...
# V2: Year-by-year projection loop with dynamic rows
for yr in range(proj_years):
    ...
    # Revenue
    if yr == 0:
        set_cell(ws3, R_REVENUE, col, "=C19*(1+C5)", font=BLACK_FONT, fmt=FMT_YEN)
    else:
        set_cell(ws3, R_REVENUE, col, f"={prev_cl}{R_REVENUE}*(1+C5)", font=BLACK_FONT, fmt=FMT_YEN)

    # V2: Feature A — SGA Absolute Projection
    if USE_SGA_ABSOLUTE:
        # SGA Expense (hardcoded input value, BLUE_FONT)
        sga_val = config["proj_sga_absolute"][yr]
        set_cell(ws3, R_SGA, col, sga_val, font=BLUE_FONT, fmt=FMT_YEN)
        # Implied Operating Margin = (Revenue - SGA) / Revenue
        set_cell(ws3, R_OP_M_IMPL, col, f"=({cl}{R_REVENUE}-{cl}{R_SGA})/{cl}{R_REVENUE}",
                 font=BLACK_FONT, fmt=FMT_PCT)
        # EBIT = Revenue - SGA
        set_cell(ws3, R_EBIT, col, f"={cl}{R_REVENUE}-{cl}{R_SGA}", font=BLACK_FONT, fmt=FMT_YEN)
    else:
        # V1 logic: EBIT = Revenue * Operating Margin
        set_cell(ws3, R_EBIT, col, f"={cl}{R_REVENUE}*C6", font=BLACK_FONT, fmt=FMT_YEN)

    # V2: Feature B — Tax with optional NOPAT floor
    if USE_TAX_FLOOR:
        set_cell(ws3, R_TAX, col, f"=MAX(0,{cl}{R_EBIT}*C8)", font=BLACK_FONT, fmt=FMT_YEN)
    else:
        set_cell(ws3, R_TAX, col, f"={cl}{R_EBIT}*C8", font=BLACK_FONT, fmt=FMT_YEN)

    # NOPAT
    set_cell(ws3, R_NOPAT, col, f"={cl}{R_EBIT}-{cl}{R_TAX}", font=BLACK_FONT, fmt=FMT_YEN)
    # D&A
    set_cell(ws3, R_DA, col, f"={cl}{R_REVENUE}*C20", font=BLACK_FONT, fmt=FMT_YEN)
    # Capex
    set_cell(ws3, R_CAPEX, col, f"={cl}{R_REVENUE}*C7", font=BLACK_FONT, fmt=FMT_YEN)
    # UFCF
    set_cell(ws3, R_UFCF, col, f"={cl}{R_NOPAT}+{cl}{R_DA}-{cl}{R_CAPEX}", font=BLACK_FONT, fmt=FMT_YEN)
    # Discount Factor
    set_cell(ws3, R_DISC, col, f"=1/(1+C26)^{yr+1}", font=BLACK_FONT, fmt="0.0000")
    # PV of FCF
...
set_cell(ws3, R_TV_PGM, 3, f"={last_cl}{R_UFCF}*(1+C15)/(C26-C15)", font=BLACK_FONT, fmt=FMT_YEN)
...
set_cell(ws3, R_EQ_PGM, 3, f"=C{R_EV_PGM}-C18", font=BLACK_FONT, fmt=FMT_YEN)

set_cell(ws3, R_PRICE_PGM, 2, "Implied Share Price (PGM)", font=BOLD_FONT)
set_cell(ws3, R_PRICE_PGM, 3, f"=ROUND(C{R_EQ_PGM}*1000000/C17,0)", font=BLACK_FONT, fmt=FMT_YEN,
         border=TOP_BOTTOM)
...
# V2: Feature A — Year 5 EBITDA calculation differs in SGA absolute mode
set_cell(ws3, R_YR5_EBITDA, 2, "Year 5 EBITDA", font=BOLD_FONT)
if USE_SGA_ABSOLUTE:
    # EBITDA = EBIT + D&A (since EBIT is already Revenue - SGA)
    set_cell(ws3, R_YR5_EBITDA, 3, f"={last_cl}{R_EBIT}+{last_cl}{R_DA}", font=BLACK_FONT, fmt=FMT_YEN)
else:
    # V1 logic: EBITDA = Revenue * (op_margin + da_pct)
    set_cell(ws3, R_YR5_EBITDA, 3, f"={last_cl}{R_REVENUE}*(C6+C20)", font=BLACK_FONT, fmt=FMT_YEN)

set_cell(ws3, R_TV_EXIT, 2, "Terminal Value (Exit Multiple)", font=BOLD_FONT)
set_cell(ws3, R_TV_EXIT, 3, f"=C{R_YR5_EBITDA}*C16", font=BLACK_FONT, fmt=FMT_YEN)
...
set_cell(ws3, R_EQ_EXIT, 3, f"=C{R_EV_EXIT}-C18", font=BLACK_FONT, fmt=FMT_YEN)

set_cell(ws3, R_PRICE_EXIT, 2, "Implied Share Price (Exit Multiple)", font=BOLD_FONT)
set_cell(ws3, R_PRICE_EXIT, 3, f"=ROUND(C{R_EQ_EXIT}*1000000/C17,0)", font=BLACK_FONT, fmt=FMT_YEN,
         border=TOP_BOTTOM)
...
for i, comp in enumerate(comps):
    ...
    # V2: Feature C — Handle negative multiples (N/A)
    if comp["ebitda"] <= 0:
...
# V2: Feature C — EV/Sales vs EV/EBITDA for row 21
if USE_EV_SALES:
...
# V2: Feature C — EV/Sales vs EV/EBITDA implied price formula
if USE_EV_SALES:
    set_cell(ws4, 27, 2, "Via EV/Sales (Median)", font=BOLD_FONT)
    # E16 = Median EV/Revenue
