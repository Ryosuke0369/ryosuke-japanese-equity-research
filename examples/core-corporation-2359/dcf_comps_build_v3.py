...
config = {
    ...
    "revenue_growth": 0.876,
    "cogs_pct": 0.34,      # V3: COGS as % of revenue for projections
    "capex_pct": 0.03,
    "da_pct": 0.015,
    "tax_rate": 0.30,
    "risk_free": 0.022,
    ...
    "proj_sga_absolute": [1381, 1452, 1500, 1550, 1600],  # FY26E through FY30E
...
assumptions = [
    ("Revenue Growth Rate",        C["revenue_growth"],       FMT_PCT),      # C5
    ("COGS % of Revenue",          C["cogs_pct"],             FMT_PCT),      # C6
    ("Capex / Revenue",            C["capex_pct"],            FMT_PCT),      # C7
    ("Effective Tax Rate",         C["tax_rate"],             FMT_PCT),      # C8
    ("Risk-Free Rate",             C["risk_free"],            FMT_PCT),      # C9
    ("Beta",                       C["beta"],                 "0.00"),       # C10
    ("Equity Risk Premium",        C["erp"],                  FMT_PCT),      # C11
    ("Size Premium",               C["size_premium"],         FMT_PCT),      # C12
    ("After-tax Cost of Debt",     C["cost_of_debt_at"],      FMT_PCT),      # C13
    ("D/E Ratio",                  C["de_ratio"],             "0.000"),      # C14
    ("Terminal Growth Rate",       C["terminal_growth"],      FMT_PCT),      # C15
    ("Exit Multiple (EV/EBITDA)",  C["exit_multiple"],        FMT_RATIO),    # C16
    ("Fully Diluted Shares",       C["shares_outstanding"],   FMT_INT),      # C17
    ("Net Debt (JPY mn)",          C["net_debt"],             FMT_YEN),      # C18
    ("Base Year Revenue (JPY mn)", C["base_year_revenue"],    FMT_YEN),      # C19
    ("D&A / Revenue",              C["da_pct"],               FMT_PCT),      # C20
]
...
# V3: Year-by-year projection loop — full waterfall
for yr in range(proj_years):
    ...
    # COGS = Revenue * COGS%
    set_cell(ws3, R_COGS, col, f"={cl}{R_REVENUE}*C6", font=BLACK_FONT, fmt=FMT_YEN)
...
base_gm = 1 - C["cogs_pct"]  # Base gross margin

for i, wacc in enumerate(wacc_range):
    r = 6 + i
    set_cell(ws5, r, 2, wacc, font=BOLD_FONT, fmt=FMT_PCT, border=THIN_BORDER)
    for j, tg in enumerate(tg_range):
        price = calc_dcf_pgm(C["revenue_growth"], base_gm, wacc, tg, C)
...
for i, rg in enumerate(rev_growth_range):
    ...
    for j, gm in enumerate(gm_range):
        ...
        is_base = (abs(rg - C["revenue_growth"]) < 0.002 and abs(gm - base_gm) < 0.002)
        fill = LIGHT_YELLOW if is_base else None
        set_cell(ws5, r, 3 + j, price, font=BLACK_FONT, fmt=FMT_YEN, border=THIN_BORDER,
                 fill=fill)
