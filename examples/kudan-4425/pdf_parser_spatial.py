"""
Kudan (4425) 決算短信PDF 財務データ抽出スクリプト (Spatial V3)
...
def extract_inner_page_items(pdf_path):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_idx, page in enumerate(pdf.pages):
            ...
            # --- PL items: find on pages with 損益計算書 or 売上原価 ---
            if "売上原価" in page_text:
                for search_str, key, _ in targets:
                    if _ != "pl":
                        continue
                    for w in words:
...
def extract_financials(pdf_path):
    """
    決算短信PDFの1ページ目から連結経営成績テーブルを抽出する。
    ...
    # Look for "百万円" unit row to know where data starts
    unit_y = None
    for w in words:
    ...
    data_rows = []  # list of (y, period_label)
    for w in words:
        if w["top"] > unit_y and re.search(r"\d{4}年", w["text"]):
            data_rows.append((w["top"], w["text"]))

    # Keep only the first 2 data rows (current year, previous year)
