...
| コンポーネント | パス | 移植判定 |
|:---|:---|:---|
| 日本営業日カレンダー | `common/jp_calendar.py` | **移植可**（外部依存なし。祝日は近似式。2027年以降の制度変更・特別休場は未考慮） |
...
| 機械的予測 A+B+C | `module_b/forecaster.py` | **移植可**（本体に相当機能なし） |
| 週次選定・レポート | `module_b/weekly_screener.py` | **移植可**（本体P4の参考形式） |
| 出口判定エンジン | `module_c/exit_engine.py` | **移植可**（本体に出口定義が無い空白を埋める。§5） |
| PITバックテスト | `module_d/backtest.py` | **移植可**（PIT機構は本体に無い生命線。ただし§4の改良が必要） |
| モック一式 | `mock/` | **回帰テスト資産として保持**（実データ接続後も削除しない） |
| モックのバックテスト数値 | `data/backtest_*.csv` | **移植しない**（相関を注入したモック世界の結果。エッジの証明ではない） |
...
python mock/generate_mock_financials.py  # 財務モック（全ユニバース）
...
Kimi側の全モジュールは `module_b/data_access.py` の3関数だけを経由してデータを読みます。本体統合は**この3関数を本体の単独値ビルダー出力に向き替える作業に集約**されます。
...
# 単位: 百万円。四半期単独値（累計ではない）。is_valid=1 のみ。古い順。
...
- 注意2: 訂正短信（同一 period_end の再提出）で解析値を上書きすると静かにルックアヘッドが混入する。ファクトに訂正世代を持たせ、「as_of時点で最新だった版」を再現できること
...
## 4. 既知の弱点と改良指示（そのまま移植しない箇所）

1. **前年同期の取り方が位置ベース（`pl[-5]`）**。is_valid=0 で期が欠けると前年同期でなくなる。→ 本体では `(quarter_type, fiscal_year - 1)` の**キー突合**に変更すること（対象: scorers_s1_s4.py の S1/S2、scorers_s5_s8.py の S6 等）
2. **バックテストの評価が絶対リターンのみ**。出口を T+20/T+60 に拡張する場合は市場指数・セクター指数に対する**超過リターン評価**を追加すること（さもないと「持っていただけ」効果とイベント効果が分離できない）
3. **決算後エントリー（T+1/T+5）をグリッドに載せる場合**、入口条件関数を差し替えること。決算前=証拠スコア、決算後=Module C の4分岐発火（「予想超過＋上方修正」）を転用するのが自然。エンジンの変更は `add_business_days(発表日, ∓N)` の符号と条件関数の差し替えポイント集約のみ
...
1. **比較基準の凍結**: 発表前に機械予測を `forecast_snapshots` に保存（週次スクリーナーが自動記録済み）。事後の握り直しを構造的に禁止
...
3. **4分岐ルール**（順序固定、ミス判定最優先）:
   ...
   - 95〜105%レンジ → 全利確
   - それ以外 → 撤退
4. **ガイダンス比較の定義**: 通期営業利益予想の新旧比較（新>旧=上方修正）。FY発表時は翌期初値の扱いを別途定義すること（モックで実際に不整合を踏んだ: 7月決算銘柄の通期発表では当該年度の期初予想が存在し得ない）
...
両者は同じ「S1〜S8」の名前で中身が異なる。統合後は以下の一本化を提案:
...
2. 本体の単独値ビルダー出力に対し §3-1 の3関数を満たすアダプタを作成（item_key 写像表を config に1枚追加）
3. §3-2 の PIT 規則（filing_date 基準・訂正世代）を本体のファクト層に実装
4. §6-1 の調整レイヤー（pl_adjustments 相当）を新設
5. `module_a`（決算カレンダー）を本体 filings に接続し、精度を前向き検証する記録を開始
6. `module_d`（バックテスト）を §4-1/4-2 の改良付きで移植し、実データで中核仮説（S2/S3/S4発火→次四半期PL改善の先行性）を検証
