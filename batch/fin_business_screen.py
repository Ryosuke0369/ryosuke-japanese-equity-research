...
def classify(hits, total):
    ...
    この規則が【できないこと】が2つある。追補15 A-0 の全件再スクリーンで確認した。

    (1) 手数料型の銀行を型D と判定できない。8410 セブン銀行は貸出金が総資産の 5.9%
        しかない(収益の柱が ATM 手数料で、資産の大半が ATM 現金と預け金)ため、
        資産比の規則では型E に見えてしまう。銀行かどうかは財務諸表の様式
        (銀行様式=経常収益・経常利益、...BNK タグ)で決まるのであって比率では決まらない。
        **company_type が既に宣言されている銘柄では、宣言のほうが優先される。**

    (2) captive finance(型C)と銀行連結(型E)を区別できない。6326 クボタは金融資産が
        35.8% あるが、その中身はクボタ製品の販売金融であって銀行ではない。
        自社製品の販売金融か第三者向けの金融かは科目名からは分からない。
    """
    ...
    # 重要性の下限。リース債権や割賦売掛金が数十億円あるだけの製造業・サービス業まで
    # 型E と言ってしまうと、型判定が意味を失う。全件再スクリーン(追補15 A-0)では
    # セコム 2.9% / 大阪ガス 2.0% / 東京ガス 0.5% / ANAHD 0.2% / ヤマトHD がこれに当たり、
    # いずれも本業に金融事業を持たない。検出事実は返すが型は変更しない。
    if share < 0.05:
        return {"type": None, "share": share,
                "liab_share": sum(liabs.values()) / total, "unknown": unknown,
                "reason": f"金融資産が総資産の {share:.1%} で 5% 未満。検出はしたが型は変更しない"
                          f"（リース債権・割賦売掛金の通常残高とみなす）"}
