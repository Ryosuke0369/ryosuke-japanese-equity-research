<#
    screener/backup_data.ps1 — DATA_ROOT (D:\screener_data) の差分ミラー + 世代バックアップ。
...
# ---- 事前確認2: 空き容量 -------------------------------------------------
$srcSize = (Get-ChildItem $src -Recurse -File -ErrorAction SilentlyContinue |
...
foreach ($d in @($current, $snapRoot, $logDir)) {
    if (-not (Test-Path $d)) {
...
if ($needSnap) {
    ...
    $old = Get-ChildItem $snapRoot -Directory | Sort-Object Name -Descending |
