...
## 2026-09-21 シャドウI-v2（進捗率の季節性）と、キャッシュ穴埋めの訂正

- [x] **訂正（§50）**: 9/20 の「キャッシュ穴埋め完了」は別期間（2024-12〜2025-09）を埋めていた。
      `--fetch-jquants` は `--from/--to` を窓として扱い、取得は「窓−400〜−330日」固定だったため。
      `--fetch-from/--fetch-to` を追加し、本来の 2026-02〜07（130日）を埋め直した
- [x] 日中足は現契約で不可（§48）。`/equities/bars/daily/am` は「not available on your subscription」
- [x] 上方修正サンプルの5年拡張は成立（§49）。決算サマリーに予想値と開示日時がある
- [x] **シャドウI-v2 を事前登録**: 分母を5年中央値、判定量を R（= 推定着地 ÷ 会社予想）。R ≥ 1.30
- [x] `fins_summary` に5年取り込み（94,758行 / 4,346銘柄 / 1,226営業日 / 約28分）
- [x] **分布を測定（§51）**: R 計算可 1,185/1,655。過去中央値進捗率 Q1 23.6% / Q2 48.0% / Q3 74.8%。
      100%超は 3Q 48社・2Q 11社・1Q 3社（保守的な予想の代理変数）。R ≥ 1.30 は 257銘柄（21.7%）
- [x] **3441 山王は R 1.20 で発火しない**（進捗113%・自社中央値94%）。設計の出発点だった実例と整合
- [ ] 次: 予測力の測定（I2-4 の 3）。G と同じ枠組み＋母集団を揃えた版。**5年分の修正イベントで測れる**
- [ ] 申し送り: R ≥ 1.30 が 21.7% と多い。開示義務の「30%」と「推定着地の乖離30%」は同じではない

## 2026-09-21 追記(2): S5c の予測力（5年）・完了条件の突合・初動の5年再測定の可否

- [x] **シャドウI-v2 追補を事前登録 → 測定（§52）**。修正イベント 13,458件（up 8,111 / down 5,347）
- [x] **R ≥ 1.30 の up率 30.93% 対 母集団 11.97%（+18.95pt・判定日クラスタ t 7.51）。**
      S5（+0.39pt）とは対照的。R 水準別の precision は 5.9%→36.1% と単調（診断・閾値は不変）
- [x] **「保守的な会社」は上方修正を出さない**（過去中央値進捗率100%超の up率 6.73% 対 90%未満 12.37%）。
      §51 の申し送りへの回答は「同じではない・逆方向」
- [x] **3件目の訂正を受けて `common.verify_range()` を実装（§53）**。
      取得後に要求範囲と実際を突合し、欠けがあれば `incomplete` + 終了コード1。
      jquants_fins / jquants_prices / earnings_window に組み込み。テスト5件
- [x] `fins_summary` に開示日時があることを確認（§54）。**初動・ギャップ分解の5年再測定は可能**
- [ ] **承認待ち**: 初動の5年再測定（H の定義のまま・規模別と場中の切り口を追加登録してから）
- [ ] 申し送り: R の効果が「保守的な会社を避けていること」から来ていないかの切り分け（§52 末尾）
- [ ] 申し送り: tdnet_archiver / edinet_bulk にも verify_range を通す（次に触るとき）

## 2026-09-21 追記(3): R のトレード成績（シャドウK）と 初動の5年再測定（H-5y）

- [x] **シャドウK を事前登録 → 測定（§55）**。判定日の終値で建て次の判定日で出る固定1か月・往復0.4%
- [x] **結論: 予測力はあるが取れない。** R≥1.30 のクラスタ平均 +0.71% / **t 0.64**（基準は t≥2）、
      TOPIX超過 **−0.39%**、MDD −19.15%。R<1.30（+0.66%）との差はほぼ無い
- [x] 内訳: 保有中に上方修正が出た 294件（10.5%）は +9.23%・超過 +6.02%（t 3.46）だが、
      **修正なし 2,496件（+1.05%・超過 −1.10%）が薄めきっている**。90日窓の30.9%は1か月では10.5%
- [x] R帯別: 1.3–1.5 は t 2.03 だが **2.0+ は t −0.04・NAV 0.928・MDD −27.7%**。
      **precision が最高の帯が成績は最悪**
- [x] **シャドウH-5y（§56）**: 上方修正 3,827件（61件→63倍）。**引け後開示は寄付後 +0.00%（t −1.55）**で
      §44 の結論が5年でも再現。年別ではギャップが +0.32%（2022）→ +1.25%（2026）と拡大
- [ ] **ユーザー判断待ち**: S5c をスコアに接続するか。§55 は「接続の材料としては負」
- [ ] 申し送り: 「修正の発表日に合わせた出入り」を測るなら別の事前登録が要る（K は固定1か月）
- [ ] 申し送り: H-5y のカバー率は 43.9%（ユニバース外 7,540件）。全市場で測るなら株価の追加取得が要る

## 2026-09-21 追記(4): シャドウL（事象出口）→ 修正先回り型は不成立で確定

- [x] **シャドウL を事前登録 → 測定（§57）**。N=5/10/20 の3点、up/down いずれでも降りる形が主判定
- [x] **3点とも基準を満たさない**（クラスタ平均 −0.58% / −0.38% / +0.02%、t は −0.64 / −0.43 / 0.02）。
      up のみで降りる副案も同じ。TOPIX超過は3点ともマイナス
- [x] 修正で降りた建玉だけなら強い（N=20 で +5.71%・超過 +3.23%・t 5.10）が、**入口の10.7%しか出ない**
- [x] **確定: 修正先回り型は現行データでは成立しない。** 予測力（t 7.51）は本物だが取り方が未発見
- [x] **S5c はスコアに接続しない。0点・表示のみで記録を継続**（ユーザー判断）
- [ ] **ユーザー判断待ち**: H-5y のカバー率 43.9% を上げるためのユニバース外株価の取得（本測定の結果を見てから、との指示）
- [ ] 将来やるなら事前登録が要るもの: 開示翌朝に建てる形 / 入口の絞り込み（事後最適化を避ける設計）/ 日中足

## 2026-09-21 追記(5): 6741 日本信号を S5c に通す / GitHub 公開前の棚卸し

- [x] **6741 の R は計算不能**（過去4年の Q1 がすべて営業赤字 → 進捗 5%未満で除外 → 分母0年）。発火しない。
      R_sales（記録のみ）1.25。6742 / 6743 も R 計算不能（それぞれ 0年 / 2年）。メモ `docs/analysis/6741_s5c_seasonality_20260921.md`
- [x] 個別銘柄の内訳レポート `screener/report/seasonality_profile.py` + テスト3件（年ごとの進捗・ばらつき・単独値。除外理由も表示）
- [x] 受注・受注残は短信本文と有報の「受注実績」表から転記（XBRL タグ化されていない）。Q1 受注残は開示なし
- [x] **公開前の安全確認**: 2359 の Entry Price（取得単価）2127 が origin/main に公開済みだった
      （template の demo 設定・verify スクリプト・2359 market_analysis v2 xlsx 2本）。**今後の版からマスク**（ユーザー判断・履歴は書き換えない）
- [x] 未 push 履歴から yfinance 生ダンプ（batch/cache・batch/draft・analysis_all.txt）・ログ・CSV を除去し `.gitignore` に追加（ユーザー判断）。
      data/comps の CSV は追跡外に（手元のファイルは残る）
- [x] `docs/バックアップ運用手順書.md` のユーザー名入り絶対パスを `%USERPROFILE%` に置換してからコミット
- [x] CLAUDE.md に S5c / verify_range / 公開の境界を追記、README に screener の節を追加、
      backtest_acceptance_criteria 末尾に「登録と結果の所在」索引（シャドウD は §35b にのみ登録されていた）
- [ ] 申し送り: 期末偏重の受注産業は R が構造的に計算できない（Q1〜Q3 赤字）。定義は変えない。対象外として件数を数えるだけ
