...
### Trailing Tabs/Spaces問題
- comps CSVファイル（`data/comps/*.csv`）のティッカー値にtrailing tabs（`\t`）やspaces（` `）が混入すると、テンプレートのComps読み込みが失敗し、Compsシートが空（#NUM!エラー）になる
- 原因: Excel編集時の自動付加、comps_fetcher.pyの出力フォーマット、手動コピペ時の混入
- **generate_dcf.py実行前に必ずCSVをクリーンアップすること**

### クリーンアップ手順
generate_dcf.py実行前に以下を実行:
```bash
# 全comps CSVのtrailing whitespaceを除去
for f in data/comps/*.csv; do
  sed -i 's/[[:space:]]*$//' "$f"
done
```

### または generate_dcf.py内で自動クリーンアップ
将来的にはgenerate_dcf.pyのCSV読み込み部分で `.strip()` を適用するのが恒久対策。
